...
async def create_categoria(categoria: db_module.schemas.Categoria, db: Session = Depends(get_db)):
...
def update_categoria(id_categoria: int, categoria: db_module.schemas.Categoria, db: Session = Depends(get_db)):
    db_categoria = db_module.crud_categoria.get_categoria(db, id_categoria=id_categoria)
    if db_categoria is None:
        raise HTTPException(status_code=404, detail="Categoria not found")
